...
> sensitive personal data ตาม PDPA มาตรา 26 ทั้ง 4 อย่างนี้ต้องให้ตรวจ:
...
- [ ] `CONSENT_VERSION` = **v2** — ขยับจาก v1 ตอนเพิ่มบรรทัด leaderboard เพราะมีบัญชี
      ยินยอมข้อความเดิมไปแล้วใน prod (คนที่ยินยอม v1 ไม่เคยเห็นประโยคนั้น จะนับว่ายินยอมไม่ได้)
      · ไม่ได้ตั้งเป็น env ที่ Cloud Run — ค่า default ใน `config.py` คือค่าที่ใช้จริง
      · **ขยับเป็น v3 อีกครั้งเมื่อ DPO แก้ถ้อยคำ** — เปลี่ยนข้อความเมื่อไหร่ เปลี่ยนเวอร์ชันเมื่อนั้น
- [ ] บรรทัด leaderboard — DPO ตรวจแล้ว (ดูกรอบด้านบน)
