...
the club is* stays — members, campaigns, rewards and their stock, announcements, and
every consent, screening, health record and audit row.
...
# What production is expected to hold. These are a fingerprint as much as a rule: a
# database with different rewards in it is not the database this script was written for,
# and the gate stops rather than guessing.
...
def _snapshot(connection: Connection) -> dict[str, Any]:
    ...
    counts["reward_stock"] = {
        str(row.id): row.stock
        for row in connection.execute(text("SELECT id, stock FROM reward")).all()
    }
    return counts
...
def _verify(
    ...
    if after["reward_stock"] != before["reward_stock"]:
        failures.append("reward stock changed row by row")
